...
int main( int argc, const char ** argv )
{
	...
	if ( argc > 1 ) {
		...
		XMLElement* quizz_root = doc->RootElement();
		XMLElement* quizz = quizz_root->FirstChildElement("quizz");
		...
		for (int index = 0; index < 100000; index++)
		{
			...
			const char *catagory_txt = answer->GetText();

			printf("Question: %s\n", question_txt);
			printf("Answer: %s\n", answer_txt);
			printf("Catagory: %s\n", catagory_txt);

			quizz = quizz->NextSiblingElement("quizz");
